...
django.setup()
from Databases.WeatherDB.models import Countries, Dates, Counties, LatitudeLongitude, WeatherStations, WeatherSensors, \
    SensorType
...
def parse_alert1_all_sensors(file, alert_summary_dict, start_row):
    ...
    for row in sheet.iter_rows(min_row=start_row):
        ...
        print(data_dict)
        date_station, date_station_created = Dates.objects.using('WeatherDB').get_or_create(
            date=alert_summary_dict[data_dict['station_name']]['date_installed'])
        ...
        lat_lon, lat_lon_created = LatitudeLongitude.objects.using('WeatherDB').get_or_create(
            latitude=data_dict['lat_lon'][0],
            longitude=data_dict['lat_lon'][1]
            )
        if lat_lon_created:
            lat_lon.save()

        weather_station, weather_station_created = WeatherStations.objects.using('WeatherDB').get_or_create(
            name=data_dict['station_name'],
            lat_lon=lat_lon,
            country=country,
            county=county,
        ...
        sensor_type, sensor_type_created = SensorType.objects.using('WeatherDB').get_or_create(
            sensor_type=data_dict['dev_type'])
        if sensor_type_created:
            sensor_type.save()

        weather_sensors, weather_sensors_created = WeatherSensors.objects.using('WeatherDB').get_or_create(
...
if __name__ == '__main__':
    alert_summary_dict = parse_alert_station_summary(
        '/home/chase/DissertationProjects/ArboModel/ArboModelData/Databases/WeatherDB/Weather_Data_Scraping/ALERT_Data/ALERT2_Numbering_Summary.xlsx')
    parse_alert1_all_sensors(
        '/home/chase/DissertationProjects/ArboModel/ArboModelData/Databases/WeatherDB/Weather_Data_Scraping/ALERT_Data/ALERT_sensors_all_by_ID.xlsx',
        alert_summary_dict,
        2)
